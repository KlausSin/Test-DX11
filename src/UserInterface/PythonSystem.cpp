...
void CPythonSystem::GetDisplaySettings()
{
	...
	// Use DXGI to enumerate display modes (replaces D3D9 adapter mode enumeration)
	...
	if (numModes > 0)
	{
		...
		pOutput->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, 0, &numModes, modes.data());
		...
		for (UINT iMode = 0; iMode < numModes; iMode++)
		{
			...
			DWORD refreshRate = mode.RefreshRate.Denominator ? (mode.RefreshRate.Numerator / mode.RefreshRate.Denominator) : 0;
