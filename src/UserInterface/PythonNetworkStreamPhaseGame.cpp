...
#ifdef ENABLE_INSTANT_WARP
bool CPythonNetworkStream::RecvWarpLocalPacket()
{
	TPacketGCWarpLocal kPacket;
...
{
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetShopSellingPrice", Py_BuildValue("(L)", *(int64_t *)&buf[0]));
	return true;
}
...
bool CPythonNetworkStream::RecvExchangeSub_ItemAdd(const TPacketGCExchange& pack)
{
	...
	if (pack.is_me)
	{
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_AddItemToExchange",
			Py_BuildValue("(iii)", pack.arg4.window_type, pack.arg4.cell, pack.arg2.cell));
	}
#endif
...
bool CPythonNetworkStream::SendAcceClosePacket()
{
	...
	TPacketAcce sPacket;
	sPacket.header = CG::ACCE;
	sPacket.length = sizeof(sPacket);
	sPacket.subheader = AcceSub::CG::CLOSE;
...
bool CPythonNetworkStream::SendAcceAddPacket(TItemPos tPos, BYTE bPos)
{
	...
	sPacket.header = CG::ACCE;
	sPacket.length = sizeof(sPacket);
	sPacket.subheader = AcceSub::CG::ADD;
	sPacket.bPos = bPos;
	sPacket.tPos = tPos;
	if (!Send(sizeof(sPacket), &sPacket))
		return false;
...
bool CPythonNetworkStream::SendAcceRemovePacket(BYTE bPos)
{
	...
	sPacket.header = CG::ACCE;
	sPacket.length = sizeof(sPacket);
	sPacket.subheader = AcceSub::CG::REMOVE;
	sPacket.bPos = bPos;
	if (!Send(sizeof(sPacket), &sPacket))
		return false;
...
bool CPythonNetworkStream::SendAcceRefinePacket()
{
	...
	sPacket.header = CG::ACCE;
	sPacket.length = sizeof(sPacket);
	sPacket.subheader = AcceSub::CG::REFINE;
	if (!Send(sizeof(sPacket), &sPacket))
		return false;
