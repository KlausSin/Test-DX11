...
void CActorInstance::ProcessMotionEventEffectEvent(const CRaceMotionData::TMotionEventData* c_pData)
{
	...
	if (c_pEffectData->isIndependent)
	{
		...
		XMFLOAT4X4 matLocalPosition;
		XMStoreFloat4x4(&matLocalPosition, XMMatrixTranslation(
...
void CActorInstance::ProcessMotionEventEffectToTargetEvent(const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	if (c_pEffectToTargetData->isFishingEffect)
	{
		...
		m_iFishingEffectID = rkEftMgr.CreateEffect(c_pEffectToTargetData->dwEffectIndex, m_v3FishingPosition, XMFLOAT3(0.0f, 0.0f, 0.0f));
	}
	else
	{
		...
		if (c_pEffectToTargetData->isFollowing && IsFlyTargetObject())
		{
			...
			XMFLOAT3 v3Position(	c_pEffectToTargetData->v3EffectPosition.x,
		...
		{
			const XMFLOAT3 & c_rv3FlyTarget = m_kFlyTarget.GetFlyTargetPosition();
			XMFLOAT3 v3Position(	c_rv3FlyTarget.x + c_pEffectToTargetData->v3EffectPosition.x,
								c_rv3FlyTarget.y + c_pEffectToTargetData->v3EffectPosition.y,
								c_rv3FlyTarget.z + c_pEffectToTargetData->v3EffectPosition.z);
			CEffectManager::Instance().CreateEffect(c_pEffectToTargetData->dwEffectIndex, v3Position, XMFLOAT3(0.0f, 0.0f, 0.0f));
		}
	}
}
...
void CActorInstance::ProcessMotionEventWarp(const CRaceMotionData::TMotionEventData* c_pData)
{
	...
	if (m_kFlyTarget.IsValidTarget())
	{
		XMFLOAT3 v3MainPosition(m_x, m_y, m_z);
		const XMFLOAT3& c_rv3TargetPosition = __GetFlyTargetPosition();

		XMFLOAT3 v3Distance;
		XMStoreFloat3(&v3Distance, XMVector3Normalize(XMLoadFloat3(&c_rv3TargetPosition) - XMLoadFloat3(&v3MainPosition)));

		TPixelPosition DestPixelPosition;
		XMStoreFloat3(&DestPixelPosition, XMLoadFloat3(&c_rv3TargetPosition) - XMLoadFloat3(&v3Distance) * sc_fDistanceFromTarget);
