...
void CTerrain::LoadMiniMapTexture(const char * c_pchMiniMapFileName)
{
	...
	{
		m_lpMiniMapTexture = m_MiniMapGraphicImageInstance.GetTexturePointer()->GetSRV();
...
void CTerrain::LoadShadowTexture(const char * ShadowFileName)
{
	...
	if (!m_ShadowGraphicImageInstance.GetTexturePointer()->IsEmpty())
		m_lpShadowTexture = m_ShadowGraphicImageInstance.GetTexturePointer()->GetSRV();
	else
	{
...
ID3D11ShaderResourceView* CTerrain::AddTexture32(BYTE byImageNum, BYTE * pbyImage, long lTextureWidth, long lTextureHeight)
{
	...
	safe_release(m_lpAlphaTexture[byImageNum]);

	if (!ms_lpd3d11Device)
		return NULL;

	const UINT MIP_LEVELS = 5;
	const UINT BASE_SIZE = 256;

	// Build all mip levels into BGRA buffers (CPU side)
	std::vector<std::vector<DWORD>> mipData(MIP_LEVELS);

	// Mip 0 — downsample 258x258 input to 256x256 alpha values, expand to BGRA
	{
		mipData[0].resize(BASE_SIZE * BASE_SIZE);
		BYTE alpha[BASE_SIZE * BASE_SIZE];

		BYTE* pbDstPixel = alpha;
		BYTE* abCurLine = pbyImage;
		for (UINT y = 0; y < 256; ++y, abCurLine += 258)
		{
			for (UINT x = 0; x < 256; ++x)
			{
				BYTE* pbSrcPixel = abCurLine + x;
				*pbDstPixel++ =
		...
		// Expand alpha → BGRA (alpha in A channel, RGB white)
		for (UINT i = 0; i < BASE_SIZE * BASE_SIZE; ++i)
			mipData[0][i] = (alpha[i] << 24) | 0x00FFFFFF;
	}

	// Mips 1..4 — box filter downsample
	UINT srcSize = BASE_SIZE;
	for (UINT level = 1; level < MIP_LEVELS; ++level)
	{
		UINT dstSize = srcSize >> 1;
		mipData[level].resize(dstSize * dstSize);

		const DWORD* src = mipData[level - 1].data();
		DWORD* dst = mipData[level].data();

		for (UINT y = 0; y < dstSize; ++y)
		{
			for (UINT x = 0; x < dstSize; ++x)
			{
				const DWORD* p = src + (y * 2) * srcSize + (x * 2);
				BYTE a = (BYTE)(((p[0] >> 24) + (p[1] >> 24) + (p[srcSize] >> 24) + (p[srcSize + 1] >> 24)) >> 2);
				dst[y * dstSize + x] = (a << 24) | 0x00FFFFFF;
			}
		}
	...
	D3D11_SUBRESOURCE_DATA initData[MIP_LEVELS];
	UINT mipSize = BASE_SIZE;
	for (UINT i = 0; i < MIP_LEVELS; ++i)
	{
		initData[i].pSysMem = mipData[i].data();
		initData[i].SysMemPitch = mipSize * 4;
		initData[i].SysMemSlicePitch = 0;
		mipSize >>= 1;
	}

	ID3D11Texture2D* pTex = NULL;
	HRESULT hr = ms_lpd3d11Device->CreateTexture2D(&td, initData, &pTex);
	if (FAILED(hr))
	{
		TraceError("CTerrain::AddTexture32 - CreateTexture2D failed (0x%08X)", hr);
		return NULL;
	}

	ID3D11ShaderResourceView* pSRV = NULL;
	hr = ms_lpd3d11Device->CreateShaderResourceView(pTex, NULL, &pSRV);
	pTex->Release();
...
void CTerrain::AllocateMarkedSplats(BYTE * pbyAlphaMap)
{
	...
	safe_release(m_lpMarkedTexture);

	if (!ms_lpd3d11Device)
		return;

	// Convert alpha map to BGRA
	std::vector<DWORD> bgra(ATTRMAP_XSIZE * ATTRMAP_YSIZE);
	for (UINT i = 0; i < ATTRMAP_XSIZE * ATTRMAP_YSIZE; ++i)
		bgra[i] = (pbyAlphaMap[i] << 24) | 0x00FFFFFF;

	D3D11_TEXTURE2D_DESC td = {};
	td.Width = ATTRMAP_XSIZE;
	td.Height = ATTRMAP_YSIZE;
	td.MipLevels = 1;
	td.ArraySize = 1;
	td.Format = DXGI_FORMAT_B8G8R8A8_UNORM;
	td.SampleDesc.Count = 1;
	...
	ID3D11Texture2D* pTex = NULL;
	if (FAILED(ms_lpd3d11Device->CreateTexture2D(&td, &initData, &pTex)))
		return;

	HRESULT hr = ms_lpd3d11Device->CreateShaderResourceView(pTex, NULL, &m_lpMarkedTexture);
	pTex->Release();
