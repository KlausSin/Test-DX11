...
PyObject * playerGetStatus(PyObject* poSelf, PyObject* poArgs)
{
	...
	int64_t iValue = CPythonPlayer::Instance().GetStatus(iType);
...
{
	int iOpen;
	if (!PyTuple_GetInteger(poArgs, 0, &iOpen))
		return Py_BuildException();

	CPythonPlayer::Instance().SetAcceRefineWindowOpen(iOpen != 0);
	return Py_BuildNone();
}

PyObject * playerGetAcceRefineWindowOpen(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonPlayer::Instance().GetAcceRefineWindowOpen() ? 1 : 0);
}
#endif
...
PyObject * playerRegisterEmotionIcon(PyObject* poSelf, PyObject* poArgs)
{
	...
	CGraphicImage * pImage = CResourceManager::Instance().GetTyped<CGraphicImage>(szFileName);
