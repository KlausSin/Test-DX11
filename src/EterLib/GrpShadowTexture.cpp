...
void CGraphicShadowTexture::Begin()
{
	...
	STATEMANAGER.GetSampler().SetFilter(3, D3D11_FILTER_MIN_MAG_MIP_POINT);
	STATEMANAGER.GetSampler().SetAddressUV(3, D3D11_TEXTURE_ADDRESS_CLAMP, D3D11_TEXTURE_ADDRESS_CLAMP);

	STATEMANAGER.SetTexture(4, NULL);
	STATEMANAGER.GetSampler().SetFilter(4, D3D11_FILTER_MIN_MAG_MIP_POINT);
	STATEMANAGER.GetSampler().SetAddressUV(4, D3D11_TEXTURE_ADDRESS_CLAMP, D3D11_TEXTURE_ADDRESS_CLAMP);
}
