...
#if defined(WJ_MULTI_TEXTLINE)
static const std::vector<std::string> SplitMultiLine(const std::string& text)
{
	static const std::regex re(R"(\\n|\n)");
	const std::sregex_token_iterator first{ text.begin(), text.end(), re, -1 }, last;
	return { first, last };
}
...
void CGraphicTextInstance::Render(RECT * pClipRect)
{
	...
	static std::unordered_map<ID3D11ShaderResourceView*, std::vector<SVertex>> s_outlineBatches;
	static std::unordered_map<ID3D11ShaderResourceView*, std::vector<SVertex>> s_mainBatches;
...

	{
		...
		if (m_isOutline)
		{
			...
			for (i=m_pCharInfoVector.begin(); i!=m_pCharInfoVector.end(); ++i, ++charIdx)
			{
				...
				fFontSx = fCurX + pCurCharInfo->bearingX;
				fFontSy = fCurY;
				fFontEx = fFontSx + fFontWidth;
				fFontEy = fFontSy + fFontHeight;

				pFontTexture->SelectTexture(pCurCharInfo->index);
				std::vector<SVertex>& vtxBatch = s_outlineBatches[pFontTexture->GetSRV()];
		...
		for (int i = 0; i < (int)m_pCharInfoVector.size(); ++i)
		{
			...
			pFontTexture->SelectTexture(pCurCharInfo->index);
			std::vector<SVertex>& vtxBatch = s_mainBatches[pFontTexture->GetSRV()];
	...
	auto DrawBatchLCD = [](const std::unordered_map<ID3D11ShaderResourceView*, std::vector<SVertex>>& batches, bool skipPass2) {
